...
pub async fn read_and_prepare_webapp(
  web_happ_path: &PathBuf,
  out_path: &PathBuf,
) -> Result<(), String> {
  // 1. read the .webhapp file
  let bytes =
    fs::read(web_happ_path).map_err(|e| format!("Failed to read .webhapp file: {}", e))?;

  // decoding .webhapp file
  let web_app_bundle = WebAppBundle::unpack(std::io::Cursor::new(bytes))
  ...
  // opening ui.zip
  let file =
    fs::File::open(ui_zip_path.clone()).map_err(|e| format!("Error opening ui.zip: {:?}", e))?;

  // Unzipping ui.zip
  unzip_file(file, ui_folder_path).map_err(|e| format!("Could not unzip ui.zip: {:?}", e))?;

  // remove ui.zip after extraction
  fs::remove_file(ui_zip_path).map_err(|e| format!("Failed to remove ui.zip: {:?}", e))?;

  // Writing .happ file
  let app_id = web_happ_path
    .as_path()
    .file_stem()
  ...
  fs::write(
    out_path.join(happ_file_name).as_path(),
    app_bundle
      .pack()
      .map_err(|e| format!("Failed to pack AppBundle: {}", e))?,
  )
